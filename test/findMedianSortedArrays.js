...
var findMedianSortedArrays = function (nums1, nums2) {
    ...
    while (true) {
        ...
        if (
            (nums1[i - 1] === undefined || nums2[j] ===undefined || nums1[i - 1]<= 2 nums2[j]) && (nums2[j - 1]="==undefined" || nums1[i]="==" undefined nums2[j 1]<="nums1[i])" ) { let leftlen="i" + j, rightlen="len1" len2 i j; 左侧数量="==" 右侧数量，取 左侧最大值 和 右侧最小值 平分 if (leftlen="==" rightlen) return (max(nums1[i 1], 1]) min(nums2[j], nums1[i])) }> 右侧数量，中位数为左侧最大值
...
console.log(findMedianSortedArrays([1, 2, 3, 4, 6], [1]));</=>